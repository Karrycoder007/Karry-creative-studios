...
export function Hero() {
 ...
 useLayoutEffect(() => {
  console.log('[Hero] loaded =', loaded); // TEMP — remove once confirmed working
  if (!loaded) return;
  ...
  return (
    <section
      ref={root}
      className="relative min-h-screen flex flex-col items-center justify-center pt-24 px-6 md:px-10 overflow-hidden"
    >
      <AmbientBackground />
        ...
        <h1 className="font-display font-medium leading-[1.05]">
          <div className="hero-line overflow-hidden">
            <span className="hero-word-inner block text-[2.6rem] sm:text-6xl md:text-7xl lg:text-8xl uppercase tracking-tight">
              Craft that <em style={{ fontStyle: 'italic', color: 'var(--accent)' }}>travels</em> &amp;
            </span>
          </div>
          <div className="hero-line overflow-hidden">
            <span className="hero-word-inner block text-[2.6rem] sm:text-6xl md:text-7xl lg:text-8xl uppercase tracking-tight">
              code that <em style={{ fontStyle: 'italic', color: 'var(--accent)' }}>performs</em>
            </span>
          </div>
        </h1>
